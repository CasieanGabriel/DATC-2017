...
namespace ThumbnailGenerator
{
    public class Worker
    {
        ...
        private BitmapSource LoadImageFromStream(Stream stream)
        {
            ...
            {
                var jpegDecoder = new JpegBitmapDecoder(stream, BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.OnLoad);
                img = jpegDecoder.Frames[0];
            }
            else if (ImageFormat.Png.Equals(image.RawFormat))
            {
                var pngDecoder = new PngBitmapDecoder(stream, BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.OnLoad);
                img = pngDecoder.Frames[0];
            }
