...
namespace AlbumPhoto.Service
{
	public class AlbumFotoService
	{
		...
		public List<Poza> GetPoze()
		{
			...
			foreach (var file in query)
			{
                poze.Add(new Poza()
                {
                    ...
                    Url = file.Url,
                    SasUrl = GetContainerSasUri(_photoContainer, file.RowKey)
                });
			}
            return poze;
        }
        public List<Comment> GetComments(string pictureName)
        {
            var comments = new List<Comment>();
            var commentsQuery = (from coment in _ctx.CreateQuery<CommentEntity>(_commentsTable.Name) select coment).AsTableServiceQuery<CommentEntity>(_ctx);

            foreach (var comment in commentsQuery)
            {
                if (comment.PartitionKey.Contains(pictureName))
                {
        ...
        public void ComentOnPicture(string pictureName, string key, string comment, string user)
        {
            _ctx.AddObject(_commentsTable.Name, new CommentEntity(pictureName, key)
            {
                Text = comment,
                MadeBy = user,
            });
        ...
        static string GetContainerSasUri(CloudBlobContainer container, string fileName)
        {
